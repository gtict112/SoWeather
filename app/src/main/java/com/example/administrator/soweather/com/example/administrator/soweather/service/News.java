...
public class News {
     ...
     * 获取新闻
     */
    public void getNews(final ResponseListenter<List<New>> a, String type) {
        String key = "&key=b216d9eee8a85d2e4eb906b084a07ae8";
        String head = "http://v.juhe.cn/toutiao/index?";
        type = "type=" + type;
        String url = head + type + key;
        Request request = new Request.Builder()
                .url(url)
                .build();
        Call call = client.newCall(request);
        call.enqueue(new Callback() {
            ...
            @Override
            public void onResponse(Call call, Response response) throws IOException {
                final Result<List<New>> res = ResponseProcessUtil.getNews(response);
                try {
                    if (res.isSuccess()) {
                        a.onReceive(res);
                    } else {
